...
import { UserCommandResponseDto, UserCommandsResponseDto } from './dto/usercommand-response.dto';
...
export class UsercommandController {
  ...
  @ApiOperation({ 
    ...
    description: 'Creates a new command snippet with tags and notes for the authenticated user'
  })
  ...
  @ApiOperation({ 
    ...
    description: 'Retrieves paginated list of user commands for the authenticated user'
  })
  @ApiQuery({ name: 'page', description: 'Page number', example: 1, required: false })
  @ApiQuery({ name: 'order', description: 'Sort order', example: 'DESC', required: false, enum: ['ASC', 'DESC'] })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiBadRequestResponse({ 
    ...
    schema: {
      ...
      properties: {
        statusCode: { type: 'number', example: 400 },
        message: { type: 'array', items: { type: 'string' }, example: ['page must be a positive integer'] },
        error: { type: 'string', example: 'Bad Request' }
      }
    }
  })
  ...
  @ApiQuery({ name: 'args', description: 'Search arguments', example: 'git commit' })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiBadRequestResponse({ 
    ...
    schema: {
      ...
      properties: {
        statusCode: { type: 'number', example: 400 },
        message: { type: 'array', items: { type: 'string' }, example: ['args should not be empty'] },
        error: { type: 'string', example: 'Bad Request' }
      }
    }
  })
